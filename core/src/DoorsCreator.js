export default class DoorsCreator {
	GetDoor($door, img, doorCfg, roomCfgVersion) {
		...
		switch (roomCfgVersion) {
			...
			case 2:
				$door.style.left = `${img.clientWidth * doorCfg.x}`;
				$door.style.top = `${img.clientHeight * doorCfg.y}`;

				$door.style.height = img.clientHeight * doorCfg.h;
				$door.style.width = img.clientWidth * doorCfg.w;
